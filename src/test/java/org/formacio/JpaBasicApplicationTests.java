...
@Sql(statements= {
		"delete from t_liniesfact",
		"delete from t_factures",
...
public class JpaBasicApplicationTests {
	 ...
	 * Heu d'emprar les propietats que ja hi ha definides, no n'heu de definir 
	 * de noves a les classes.
	 * Per els ids de LiniaFactura i Factura heu d'emprar un valor generat 
	 * amb estrategia IDENTITY
	 */
	@Test
	...
	/**
	 * Modifica la classe FacturesRepositori per a que sigui un repositori
	 * Spring Data per l'entitat Factura
	 */
	@Test
	...
	/**
	 * Implementa el metode totalClient de FacturesRepositori per a que 
	 * retorni el total que s'ha gastat en totes les factures d'un client donat
	 * Per aixo, emprarem una funcio que no hem introduit:
	 *     la funcio sum () en un select retorna, com es d'esperar, la suma dels 
	 *     valors de totes les files de la select. Per tant, heu de fer una select
	 *     que retorni totes les files de les factures del client indicat i,
	 *     per cada una d'elles, sumar el valor de la propietat total
	 */
	@Test
	...
	/**
	 * Si no ho has fet, fes qeu FacturesRepositori sigui un repositori 
	 * amb els metodes CRUD automatics
	 */
	@Test
	...
	/**
	 * Implementa a FacturesRepositori emprant la tecnica de queries derivades
	 * un metode que retorni totes les factures d'un client
	 * 
	 * Fet aixo, modifica el test per tal que factures se inicialitzi 
	 * amb una invocacio al metode que has creat
	 */
     @Test
	...
	/**
	 * Modifiqueu el mapping de Factura per tal que totes les operacions 
	 * de persistencia que es fan sobre Factura es propaguin (cascade)
	 * sobre LiniaFactura
	 */
	@Test
	@Transactional
	...
	/**
	 * Modifiqueu el metode afegirProducte per tal que quan la nova linia afegida
	 * sigui la quarta o mes de la factura que modifiquem, se notifiqui al client
	 * que te un regal.
	 * 
	 * Per fer aquesta notificacio, FacturaService ha de tenir una dependencia a
	 * FidalitzacioService i invocar el metode notificaRegal passant l'email 
	 * (IMPORTANT: es l'email, no el nom del client)
